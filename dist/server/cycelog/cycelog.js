...
export function patchLog1(db, postId, body) {
    ...
    if (!(0 <= newData.type && newData.type <= 6))
...
function getMoveTarget(db, postId, startId, endId, delta) {
    ...
    if (dbResult.length !== allCount)
        throw badRequest("적용 범위에 다른 글의 1차 기록이 포함됩니다.");
    ...
    if (newRangeCount !== 0)
        throw badRequest("이 동작으로 인해 번호 충돌이 발생할 가능성이 있습니다.");
    return dbResult;
}
