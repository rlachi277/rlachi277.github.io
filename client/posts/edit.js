...
function getEditType(el) {
    switch (el.nodeName) {
        ...
        default:
            if (EDITABLES.has(el.nodeName))
                return EDIT_TYPE.EDITABLE;
            else if (CONTAINERS.has(el.nodeName))
                return EDIT_TYPE.CONTAINER;
            else if (UNITS.has(el.nodeName))
                return EDIT_TYPE.UNIT;
            else {
...
function handleSubmitKey(target) {
    ...
    if (!manageConfirm(target, "will-submit", "will-cancel"))
        return;
...
function manageConfirm(el, confirmClass, stopClass) {
    ...
    if (!el.classList.contains("edited") && !el.classList.contains("deleted"))
        return false;
...
function insertElement(after, isFirst) {
    ...
    if (after.tagName === 'LI' || ((after.tagName === 'UL' || after.tagName === 'OL') && isFirst)) {
        showWarning("리스트 편집은 아직 지원하지 않습니다.");
        return false;
    }
    if (isFirst) {
        after.insertAdjacentElement("afterbegin", newElement);
        if (newElementAddHeader)
            newElement.append(header(newElement, true));
...
export function header(after, isFirst) {
    ...
    while (cur != document.body) {
        if (cur.matches("section, article"))
            depth++;
