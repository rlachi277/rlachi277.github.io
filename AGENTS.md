...
- Building, which modifies the contents of the dist/ directory, is an exception to this rule. However, consider running `pnpm check:vibing`(ts typecheck) instead of `pnpm build:vibing` if possible.
