...
export function log1Where(db: Database, id: number): WhereData {
...
export function serverWhere(db: Database): WhereFunc {
	return function (id) {
		if (id === undefined) return {where: "void", type: 0};
		else return log1Where(db, id);
	}
}