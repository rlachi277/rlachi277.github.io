...
const code: Record<string, string[]> = {
	...
	r: ["line26", "dot30", "dot78"],
	e: ["line26", "dot30", "dot8"],
	q: ["line26", "dot30", "dot58"],
	w: ["line26", "dot0", "dot78"],
	t: ["line26", "dot0", "dot58"],
	a: ["line26", "dot10", "dot78"],
	s: ["line26", "dot10", "dot8"],
	d: ["line26", "dot10", "dot58"],
	z: ["line08", "dot12", "dot36"],
	x: ["line08", "dot12", "dot6"],
	v: ["line08", "dot12", "dot76"],
	c: ["line08", "dot2", "dot36"],
	S: ["line08", "dot2", "dot76"],
	f: ["line08", "dot52", "dot6"],
	g: ["line08", "dot52", "dot76"],
};
...
const chcode = ['r','Sr','s','e','Se','f','a','q','Sq','t','St','','w','sw','c','z','x','v','g'];
const jucode = ['k','o','i','O','j','p','u','P','h','Dk','Do','H','y','n','Dj','Dp','N','b','m','ml','l'];
const jocode = ['','r','Sr','rt','s','sw','sg','e','f','fr','fa','fq','ft','fx','fv','fg','a','q','qt','t','St','d','w','c','z','x','v','g'];
const cscode = ['','r','Sr','rt','s','sw','sg','e','Se','f','fr','fa','fq','ft','fx','fv','fg','a','q','Sq','qt','t','St','d','w','Sw','c','z','x','v','g'];

function to_code(str: string) {
	let res1 = str
		.replaceAll('hk','Dk').replaceAll('ho','Do').replaceAll('hl','H')
		.replaceAll('nj','Dj').replaceAll('np','Dp').replaceAll('nl','N')
		.replaceAll('R','Sr').replaceAll('E','Se').replaceAll('Q','Sq').replaceAll('T','St').replaceAll('W','Sw')
		.replaceAll('\\','');
	let res2 = '';
	...
	for (let i=0; i<res2.length; i++) {
		...
		if ('0'<=ch && ch<='9') {
			if ('0' <= res2[i-1] && res2[i-1] <= '9') continue;
			// 이 기호에서 숫자는 리틀 엔디안으로 나타낸다.
			// 이것은 절대로 내가 빅 엔디안을 구현하기 귀찮아서가 아니다.

			// 소수점이 있다면 이는 적히진 않지만 숫자를 끊으므로,
			// 이를테면 123.456은 (123456)이 아니라 (123)(456)과 같이 두 숫자가 붙어 있는 형태가 된다.
			// 이 기호를 사용하는 문명이 대충 그렇게 소수를 나타낸다고 가정하자.
			// 와! 512진법과 10진법 하이브리드!
